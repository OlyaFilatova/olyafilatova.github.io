...
def format_file(mappingsByUrl: dict[str, Mapping]):
  return [{
      "url": value["url"],
...
def translate_page(page):
  ...
  return zip(notes, translated)

def format_page(page, notes):
  thoughts = [{
    "uk": uk,
    "en": en
  } for en, uk in notes]
...
def create_result_path(root_dir: Path, dir_name: str):
  return root_dir / 'formatted' / f'{dir_name}'

def store_page(result_path: Path, page):
  meta, content = page
