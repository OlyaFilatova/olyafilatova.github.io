    ...
    async with postgresql_manager.get_async_session() as session:
      aggregates = (
        select(
          ...
          func.count(
            func.distinct(JobPosting.company)
          ).label("company_count"),
          ...
          func.array_agg(
            func.distinct(JobPosting.category)
          ).label("categories"),

          func.array_agg(
            func.distinct(JobPostingSkill.skill_normalized_text)
          ).filter(
            JobPostingSkill.skill_normalized_text
            != SkillSynonym.origin_normalized_text
          ).label("synonyms"),

          func.array_agg(
            func.distinct(SkillSynonym.text)
          ).filter(
            JobPostingSkill.skill_normalized_text
            != SkillSynonym.origin_normalized_text
      ...
      if search:
        where.append(
          aggregates.c.normalized_text.ilike(f"%{search.lower()}%")
        )
